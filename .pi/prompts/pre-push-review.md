...
```
IMPORTANT: Start by reading the unified diff at .claude/pre-push-review/<reviewer>-diff.txt — it shows exactly what changed (lines prefixed + are additions, - are removals) with 3 lines of context. Review the changed lines for issues. If the diff file is empty or missing, fall back to reading the full files listed in .claude/pre-push-review/<reviewer>-files.txt.
