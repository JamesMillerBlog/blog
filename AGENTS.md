...
| `security-audit.yml`         | Scheduled Saturday 06:00 UTC, manual trigger           | Weekly comprehensive audit: npm vulnerabilities (with major-bump & age-gating), licenses, secrets, markdown disclosures, source patterns (XSS, DOM injection), AI prompts (boundary exposure, auto-commit), git history (CVE messages, deleted secrets), containers, Terraform, CI/CD. Auto-fixes safe updates, reverts major bumps. Creates or updates draft security advisory, generates audit-report.md/json + audit-warnings.md, creates `ai-implement` issue if findings exist |
