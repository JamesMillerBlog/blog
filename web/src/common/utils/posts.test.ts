...
function setupFsMocks({ files = [] as string[], exists = true, fileContent = 'raw' }) {
  ...
  vi.spyOn(fs, 'readFileSync').mockReturnValue(
    fileContent as unknown as ReturnType<typeof fs.readFileSync>
  )
}
...
describe('getPostBySlug', () => {
  ...
  it('prefers the .mdx file when .mdx exists', async () => {
    setupFsMocks({ files: [], exists: true })
    mockedMatter.mockReturnValue({
      data: makeFrontmatter({ title: 'MDX Post' }),
      content: 'body',
    } as unknown as ReturnType<typeof matter>)
    const { getPostBySlug } = await import('./posts')
  ...
  it('falls back to .md when no .mdx file exists', async () => {
    ...
    mockedMatter.mockReturnValue({
      data: makeFrontmatter({ title: 'MD Post' }),
      content: 'body',
    } as unknown as ReturnType<typeof matter>)
    const { getPostBySlug } = await import('./posts')
    const post = await getPostBySlug('hello-world')
...
describe('getAllPosts — draft filtering by NODE_ENV', () => {
  ...
  it('excludes draft posts in production', async () => {
    ...
    const posts = [
      {
        slug: 'published',
        fm: makeFrontmatter({ title: 'Published', date: '2024-01-02', draft: false }),
      },
      {
        slug: 'draft-post',
        fm: makeFrontmatter({ title: 'Draft', date: '2024-01-01', draft: true }),
      },
    ]
  ...
  it('treats missing draft field as published', async () => {
    vi.stubEnv('NODE_ENV', 'production')
    const posts = [
      { slug: 'no-draft', fm: makeFrontmatter({ title: 'No draft field', date: '2024-01-01' }) },
    ]
  ...
  it('returns empty array when all posts are drafts in production', async () => {
    ...
    const posts = [
      {
        slug: 'draft-a',
        fm: makeFrontmatter({ title: 'Draft A', date: '2024-01-01', draft: true }),
      },
      {
        slug: 'draft-b',
        fm: makeFrontmatter({ title: 'Draft B', date: '2024-01-02', draft: true }),
      },
    ]
    setupFsMocks({ files: posts.map((p) => `${p.slug}.mdx`), exists: true })
    setupMatterMocks(posts)
