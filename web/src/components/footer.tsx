import Link from 'next/link'
import { ui } from '@/i18n/en'

export const Footer = (): React.JSX.Element => {
  return (
    <footer className="mt-20 rounded-t-[3rem] bg-surface-container-low py-12 px-8">
      <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center">
        {/* Branding */}
        <div className="flex flex-col items-center md:items-start gap-4 mb-8 md:mb-0">
          <p className="text-sm text-on-surface-variant font-headline">
            {ui.footer.copyright(new Date().getFullYear())}
          </p>
        </div>

        {/* Links */}
        <div className="flex gap-6">
          <FooterLink href="https://twitter.com/JamesMillerBlog" label={ui.footer.social.twitter}>
            <XIcon />
          </FooterLink>
          <FooterLink href="https://github.com/jamesmillerblog" label={ui.footer.social.github}>
            <GitHubIcon />
          </FooterLink>
          <FooterLink
            href="https://linkedin.com/in/jamesmillerblog"
            label={ui.footer.social.linkedin}
          >
            <LinkedInIcon />
          </FooterLink>
          <FooterLink href="/feed.xml" label={ui.footer.social.rss}>
            <RSSIcon />
          </FooterLink>
        </div>
      </div>
    </footer>
  )
}

const FooterLink = ({
  href,
  label,
  children,
}: {
  href: string
  label: string
  children: React.ReactNode
}): React.JSX.Element => {
  const isExternal = href.startsWith('http')
  const Component = isExternal ? 'a' : Link
  const props = isExternal ? { href, target: '_blank', rel: 'noopener noreferrer' } : { href }

  return (
    <Component
      {...props}
      aria-label={label}
      className="text-on-surface opacity-30 hover:opacity-100 transition-opacity duration-200"
    >
      {children}
    </Component>
  )
}

const XIcon = (): React.JSX.Element => {
  return (
    <svg className="w-5 h-5" viewBox="0 0 24 24" fill="currentColor">
      <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-4.714-6.231-5.401 6.231H2.744l7.73-8.835L1.254 2.25H8.08l4.253 5.622 5.911-5.622zm-1.161 17.52h1.833L7.084 4.126H5.117z" />
    </svg>
  )
}

const GitHubIcon = (): React.JSX.Element => {
  return (
    <svg className="w-5 h-5" viewBox="0 0 24 24" fill="currentColor">
      <path d="M12 2C6.477 2 2 6.484 2 12.017c0 4.425 2.865 8.18 6.839 9.504.5.092.682-.217.682-.483 0-.237-.008-.868-.013-1.703-2.782.605-3.369-1.343-3.369-1.343-.454-1.158-1.11-1.466-1.11-1.466-.908-.62.069-.608.069-.608 1.003.07 1.531 1.032 1.531 1.032.892 1.53 2.341 1.088 2.91.832.092-.647.35-1.088.636-1.338-2.22-.253-4.555-1.113-4.555-4.951 0-1.093.39-1.988 1.029-2.688-.103-.253-.446-1.272.098-2.65 0 0 .84-.27 2.75 1.026A9.564 9.564 0 0112 6.844c.85.004 1.705.115 2.504.337 1.909-1.296 2.747-1.027 2.747-1.027.546 1.379.202 2.398.1 2.651.64.7 1.028 1.595 1.028 2.688 0 3.848-2.339 4.695-4.566 4.943.359.309.678.92.678 1.855 0 1.338-.012 2.419-.012 2.747 0 .268.18.58.688.482A10.019 10.019 0 0022 12.017C22 6.484 17.522 2 12 2z" />
    </svg>
  )
}

const LinkedInIcon = (): React.JSX.Element => {
  return (
    <svg className="w-5 h-5" viewBox="0 0 24 24" fill="currentColor">
      <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 01-2.063-2.065 2.064 2.064 0 112.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z" />
    </svg>
  )
}

const RSSIcon = (): React.JSX.Element => {
  return (
    <svg className="w-5 h-5" viewBox="0 0 24 24" fill="currentColor">
      <path d="M6.18 15.64a2.18 2.18 0 012.18 2.18C8.36 19.01 7.38 20 6.18 20C4.98 20 4 19.01 4 17.82a2.18 2.18 0 012.18-2.18M4 4.44A15.56 15.56 0 0119.56 20h-2.83A12.73 12.73 0 004 7.27V4.44m0 5.66a9.9 9.9 0 019.9 9.9h-2.83A7.07 7.07 0 004 12.93V10.1z" />
    </svg>
  )
}
